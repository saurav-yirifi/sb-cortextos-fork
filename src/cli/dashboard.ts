...
export const dashboardCommand = new Command('dashboard')
  .option('--port <port>', 'Port to run dashboard on', '3010')
  ...
  .action(async (options: { port: string; instance: string; build?: boolean; install?: boolean; portProbe: boolean }) => {
    ...
    // Allow extra hostnames (Tailscale, mDNS, reverse proxies) to reach Next.js
    // dev resources. Without this, accessing the dashboard from anything other
    // than localhost blocks /_next/* and the login form sits with a spinner.
    const allowedDevOrigins =
      process.env.DASHBOARD_ALLOWED_DEV_ORIGINS ||
      dashCreds['DASHBOARD_ALLOWED_DEV_ORIGINS'] ||
      '';
    ...
    if (allowedDevOrigins) {
      nextEnvLines.push(`DASHBOARD_ALLOWED_DEV_ORIGINS=${allowedDevOrigins}`);
    }
    ...
    const dashEnv = {
      ...
      ...(allowedDevOrigins && { DASHBOARD_ALLOWED_DEV_ORIGINS: allowedDevOrigins }),
    };
