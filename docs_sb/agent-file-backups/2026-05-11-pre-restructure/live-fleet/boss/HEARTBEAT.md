...
## Step 3a: Disk pressure check (lessons from 2026-05-07 outage)

A full data volume silently kills every Bash call (ENOSPC writing tool output). Catch it before it bites.
...
if [ "$DISK_PCT" -ge 98 ]; then
  cortextos bus send-telegram $CTX_TELEGRAM_CHAT_ID "URGENT — disk at ${DISK_PCT}% (${DISK_FREE} free). Bash will start failing within minutes. Clear ~5Gi NOW: empty Trash, delete ~/Library/Caches/*, check ~/Library/Developer/Xcode/DerivedData."
  cortextos bus log-event milestone disk_critical warning --meta "{\"pct\":\"$DISK_PCT\",\"free\":\"$DISK_FREE\"}"
elif [ "$DISK_PCT" -ge 95 ]; then
  cortextos bus send-telegram $CTX_TELEGRAM_CHAT_ID "Disk warning: ${DISK_PCT}% (${DISK_FREE} free). Worth clearing ~10Gi this week."
  cortextos bus log-event milestone disk_warning info --meta "{\"pct\":\"$DISK_PCT\",\"free\":\"$DISK_FREE\"}"
fi
```

Page-direct-to-Saurav at ≥98% (no boss decision-making, this can break the system in minutes). Warn-only at 95–97%.

## Step 3c: Fleet-wide context relay sweep (Protocol D2, v1.2)

Safety net for D2: catch any Saurav-direct fleet-wide messages that arrived since last heartbeat without being propagated yet. The real-time path is propagating within 60s of receipt; this sweep catches anything missed (long tool runs, race conditions, mid-restart receipts).

```bash
# 1. Pull Saurav-direct inbound since last heartbeat (~4h window).
SINCE=$(date -u -v-4H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -d '4 hours ago' +%Y-%m-%dT%H:%M:%SZ)
INBOUND=/Users/sauravb/.cortextos/$CTX_INSTANCE_ID/logs/$CTX_AGENT_NAME/inbound-messages.jsonl

# 2. For each Saurav-direct message in window matching trigger keywords, check if
#    fleet_context_propagated event exists with that telegram_msg_id. If not, propagate now.
...
jq -c --arg since "$SINCE" 'select(.timestamp > $since and (.text | test("'"$TRIGGER_RE"'"; "i")))' "$INBOUND" | while read -r MSG; do
  ...
  # Check propagation. If no fleet_context_propagated event with this msg_id, broadcast.
  # (Implementation: scan today's events, propagate via D2 from CLAUDE.md if missing.)
done
```

Trigger keywords: based in, flying, flight, land, landed, back online, offline, afk, in meeting, out for, vacation, OOO, timezone, work hours, focus, priority, goal, travel, schedule, urgent — plus heuristic on city/time-of-day/date-range mentions.

Judgment-call rule: if a Saurav-direct message contains a keyword but is genuinely targeted (e.g. "engineer's RFC-001 priority changed"), log `fleet_context_propagated` with `propagated_to: []` and a `targeted_not_fleet_wide` reason. Analyst's watch accepts that as legitimate.

Full spec: `orgs/sb-personal/agents/analyst/specs/integrity-protocols-v1.md` (Protocol D2).
