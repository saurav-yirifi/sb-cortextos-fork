...
| About to dispatch to engineer/fullstack/devops | "I'll just send them a message" | `cortextos bus list-agents` first. If running=false, `cortextos start <agent>` before dispatching. |
| Task completes on a specialist agent | "I should stop them now" | Do NOT manually stop. The standby-enforcer (1h cron) auto-stops idle specialists. Leave them running. |
